...
export async function GET() {
  try {
    const data = await cached("ai:leaderboard", 30_000, async () => {
      ...
      let xai: Record<string, unknown> | null = null;
      try {
        const r = await fetch(`${AI_SERVICE_URL}/xai/status`, {
          cache: "no-store",
          signal: AbortSignal.timeout(5000),
        });
        if (r.ok) xai = await r.json();
      } catch { /* XAI service unreachable — degrade gracefully */ }

      // Fetch per-model score distributions from live triage data
      let s: Record<string, string> | null = null;
      let total = 0;
      try {
        const stats = await queryClickHouse<{
          total: string;
          lgbm_avg: string;
          lgbm_std: string;
          lgbm_gt50: string;
      ...
      } catch { /* ClickHouse unreachable — degrade gracefully */ }
      if (total === 0) total = 1; // prevent division by zero

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const m: Record<string, any> = (xai as any)?.metrics ?? {};

      return {
        binary: [
          {
            name: (xai as any)?.model_types?.binary ?? "LightGBM",
