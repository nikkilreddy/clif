...
const CH_HOST = process.env.CH_HOST || process.env.CLICKHOUSE_HOST || "localhost";
const CH_PORT = process.env.CH_PORT || process.env.CLICKHOUSE_PORT || "8123";
const CH_USER = process.env.CH_USER || process.env.CLICKHOUSE_USER || "default";
const CH_PASSWORD = process.env.CH_PASSWORD || process.env.CLICKHOUSE_PASSWORD || "";
const CH_DB = process.env.CH_DB || process.env.CLICKHOUSE_DB || "clif_logs";
...
/**
 * Execute a non-SELECT statement (INSERT, ALTER, etc.) that returns no rows.
 * Reuses the same connection params, circuit breaker, and retry logic.
 */
export async function executeClickHouse(sql: string): Promise<void> {
